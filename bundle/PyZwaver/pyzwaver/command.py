...
def NodeDescription(basic_generic_specific):
    ...
    if v is None:
        logging.debug("unknown generic device : %s",
...
def _ParseColorValues(m, index):
    size = m[index]
    end = 1 + 2*size + index
    return end, [(m[i], m[i+1]) for i in range(index + 1, end, 2)]
...
def _MakeColorValues(v):
    return [len(v)] + [int(x) for [component, value] in v for x in [component, value]]
...
_PARSE_ACTIONS = {
    ...
    #"C": (_ParseCommands, _MakeGroups),
    "D": (_ParseDate, _MakeDate),
    ...
    "c": (_ParseColorValues, _MakeColorValues),
