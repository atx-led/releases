...
class Node:
    ...
    def BatchCommandSubmitFiltered(self, commands: List[tuple], priority: tuple, xmit: int):
        ...
        for key, values in commands:
            ...
            #    logging.warning("ignoring command: %s", c)
            #    continue
    ...
    def UpdateNodeFirmware(self, image: bytes, manufacturer: int, fw_id: int):
        self._image = image
        self.BatchCommandSubmitFilteredFast([(z.Firmware_UpdateMetadataPrepareGet, {
            "manufacturer": manufacturer,
            "id": fw_id,
            "checksum": 0,
            "target": 0,
            "fragment_size": 128, # XXX
            "hardware_version": 0,
            })])
