...
C("ColorSwitch", 0x33,
  Get=(0x3, "B{group}"),
  Report=(0x4, "B{group},B{level},B{target},B{duration}"),
  SupportedGet=(0x1, ""),
  SupportedReport=(0x2, "R{bits}"),
  Set=(0x5, "c{colors}"),
...
C("DoorLockLogging", 0x4C,
  ...
  Get=(0x3, "B{count}"),
  Report=(0x4, "B{count},D{date},B{type},B{user},A{code}"),
  )
...
C("Firmware", 0x7a,
  MetadataGet=(0x1, ""),
  # XXX generic list at the end, should be a word array of length n_targets,
  # followed by a 1-byte hardware version
  MetadataReport=(0x2, "W{manufacturer},W{id},W{checksum},B{upgradable},"
      "B{n_targets},W{max_fragment_size},L{data}"),
  UpdateMetadataRequestGet=(0x3, "W{manufacturer},W{id},W{checksum},B{target},"
      "W{fragment_size},B{activation},B{hardware_version}"),
  UpdateMetadataRequestReport=(0x4, "B{status}"),
  UpdateMetadataGet=(0x5, "B{n_reports},W{report_number}"),
  # XXX generic list at the end, but ends with a checksum word
  UpdateMetadataReport=(0x6, "W{report_number},L{data}"),
  UpdateMetadataStatusReport=(0x7, "B{status}"),
  UpdateMetadataPrepareGet=(0xA, "W{manufacturer},W{id},W{checksum},B{target},"
      "W{fragment_size},B{hardware_version}"),
  UpdateMetadataPrepareReport=(0xB, "B{status},W{checksum}"))
...
C("Security2", 0x9f,
  ...
  MessageEncapsulation=(0x03, "B{seq},E{extensions}"),
  KexGet=(0x04, ""),
...
def DumpDartConstants(fmt: FORMAT, string_maps=True):
    ...
    def DumpConstTuple(v, k0, k1):
        print("%s%s = (0x%02x, 0x%02x)%s" %
              (fmt.constint, v, k0, k1, fmt.terminator))
